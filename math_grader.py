...
def grade_h(model_answer: str, gt_answer: str, fast: bool = True):
    if "\\boxed" in model_answer:
        model_answer = extract_answer(model_answer)
    correct = grade_answer_mathd(model_answer, gt_answer) or grade_answer_sympy(
        model_answer, gt_answer
...
#         and completions
#         and isinstance(completions[0], dict)
#         and 'content' in completions[0]
#     ):
#         return [c['content'] for c in completions]
#     return completions

def normalize_completions(completions):
    if completions and len( completions[0]) >1 : 
        if (
            isinstance(completions, list)
            # and completions
...
def boxed_reward_fn_h(completions, solution, **kwargs) -> list[float]:
    # responses = normalize_completions(completions)#completions#[completion[0]['content'] for completion in completions]
    fast = False
    # responses = [completion[0]['content'] for completion in completions]
    responses = [
        completion[0]['content'] if 'content' in completion[0] else completion
    ...
    for model_response , gt_answer in zip(responses, solution):
        ...
        if isinstance(gt_answer, float) or isinstance(gt_answer, int):
            gt_answer = str(gt_answer)
        if isinstance(gt_answer, str) and model_answer is not None:
            is_correct = grade_h(model_answer, gt_answer, fast)
        elif isinstance(gt_answer, list) and model_answer is not None:
            is_correct = False
            for gt in gt_answer:
                is_correct |= grade_h(model_answer, gt, fast)
        print("+ is_correct", is_correct)
        print("--------------")
        if is_correct:
            r = 2.0  # Correctness reward.
